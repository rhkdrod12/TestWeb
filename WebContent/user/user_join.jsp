                ...
                <div class="col-lg-6 col-md-9 col-sm-12 join-form">
                    ...
                    <form action="req_join.user" method="post" name="join_form">
                        ...
                        <div class="form-group">
                            ...
                            <input class="form-control sel" name="phone_number1" onkeyup="check_number(this);" placeholder="010"> -
                            <input class="form-control sel" name="phone_number2" onkeyup="check_number(this);" placeholder="xxxx"> -
                            <input class="form-control sel" name="phone_number3" onkeyup="check_number(this);" placeholder="xxxx">
...
<script>
	function check_number(a){
		...
		if(!exp1.test(a.value)){
			document.getElementById('checkPhone').innerHTML='숫자만 입력가능합니다.'
	...
	function check_pw(){
		if(document.getElementById('password').value !=''){
	        if(document.getElementById('password').value==document.getElementById('password-confrim').value){
	            document.getElementById('checkPwChk').innerHTML='비밀번호가 일치합니다.'
	            document.getElementById('checkPwChk').style.color='blue';
	        }
	        else{
	            document.getElementById('checkPwChk').innerHTML='비밀번호가 일치하지 않습니다.';
	            document.getElementById('checkPwChk').style.color='red';
	        }
	    }
	}
	...
	function check(){
		...
		var exp1 = /^(?=.*[a-zA-Z])[a-zA-Z0-9]{4,12}$/;
		...
		if(!exp1.test(document.join_form.id.value)){
			document.getElementById('checkId').innerHTML='아이디는 영문포함 4~12자이어야합니다.';
		...
		if(!exp2.test(document.join_form.pw.value)){
			//alert("비밀번호는 영 대/소문자, 숫자, 특수문자 3종류 이상 조합 8자 이상입니다.");
			document.getElementById('checkPw').innerHTML='비밀번호는 영 대/소문자, 숫자, 특수문자 3종류 이상 조합 8자 이상입니다.';
		...
		if (document.join_form.pw.value != document.join_form.pwChk.value){
			document.getElementById('checkPwChk').innerHTML='비밀번호가 일치하지 않습니다.';
            document.getElementById('checkPwChk').style.color='red';
            flag = false;
		}else{
			document.getElementById('checkPwChk').innerHTML='';
		}
		
		if(document.join_form.name.value == ''){
			document.getElementById('checkName').innerHTML='이름은 필수입니다.'
		...
		if(document.join_form.phone_number1.value =='' || document.join_form.phone_number2.value =='' ||document.join_form.phone_number3.value =='' ){
			document.getElementById('checkPhone').innerHTML='핸드폰 번호는 필수입니다.'
	        document.getElementById('checkPhone').style.color='red';
			flag = false;
		}else{
			document.getElementById('checkPhone').innerHTML='';
